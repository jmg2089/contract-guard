"""문의 메일 발송. 강수현 님이 작성한 app.py 의 문의 처리를 그대로 옮겼다.

옮기면서 세 가지만 고쳤다.

  1) SMTP 계정과 비밀번호 길이를 print 로 찍던 부분을 지웠다.
     Render 로그에 그대로 쌓이고, 로그는 우리가 지운다고 사라지지 않는다.
  2) 보내는 주소에 개인 메일이 기본값으로 박혀 있던 것을 환경변수로 뺐다.
     코드에 박히면 .gitignore 와 무관하게 깃에 올라간다.
  3) 문의 본문을 로그에 남기지 않는다. 문의창에 계약서를 붙여넣는 사람이 있다.

기능과 응답 형식은 그대로다. 프론트가 기대하는 {success, message} 를 유지한다.
"""
import os
import smtplib
